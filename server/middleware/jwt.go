...
import (
	"errors"
	response2 "gin-vue-admin/api/web/tools/response"
...
func APiJWTAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		token := c.Request.Header.Get("x-token")
		if token == "" {
			response2.FailWithDetailedByToken(gin.H{"reload": true}, "90000", c)
			c.Abort()
			return
		}
		if service.IsBlacklist(token) {
			response2.FailWithDetailedByToken(gin.H{"reload": true}, "90001", c)
			c.Abort()
			return
		}
		j := NewJWT()
		...
		if err != nil {
			if err == TokenExpired {
				response2.FailWithDetailedByToken(gin.H{"reload": true}, "90002", c)
				c.Abort()
				return
			}
			response2.FailWithDetailedByToken(gin.H{"reload": true}, "99999", c)
			c.Abort()
			return
		}
